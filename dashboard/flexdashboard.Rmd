...
data("coronavirus")
dim(coronavirus)
#coronavirus <- load("../input/coronavirus.rda") # %>%
  #mutate(Country.Region = ifelse(Country.Region == "Iran (Islamic Republic of)", "Iran", Country.Region)) %>%
  #mutate(Country.Region = ifelse(Country.Region == "Republic of Korea", "South Korea", Country.Region))
...
coronavirus <- coronavirus %>% 
  ...
  mutate(Country.Region = ifelse(Country.Region == "Taipei and environs", "Taiwan", Country.Region)) %>% 
...
map <- tm_basemap(leaflet::providers$OpenStreetMap) +
  ...
  tm_polygons("total_confirmed", id = "name", 
              ...
              breaks = c(1, 10, 100, 1000, 10000, 80000, Inf)) +
...
rki_germany %>% 
  rename(`Federal State` = 1, `Confirmed Cases` = 2, `Deaths` = 3, `High Risk Areas` = 4) %>% 
